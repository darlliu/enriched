...
int main(int argc, char **argv) {
  ...
  SymSet<symbol16, annotation24> myset1({"m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12"}, mydataset);
  auto res = myset1.get();
  auto res2 = myset1.get_mapped_mask();
  auto res3 = myset1.get_mask();
  auto f1 = fisher_t(1, 9, 11, 3);
  auto f2 = fisher_t(0, 10, 12, 2);
  SymSet<symbol16, annotation24> myset2({ "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12" }, mydataset);
  auto test_res = ab_test<SymSet<symbol16, annotation24>, Dataset<symbol16, annotation24>, fisher_t>(myset1, myset2, mydataset);
  return 0;
}